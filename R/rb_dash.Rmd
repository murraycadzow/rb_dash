...
```{r}
resbaz %>% ggplot(., aes(x = Discipline, fill = Discipline)) + geom_bar() + theme_bw()+ theme(axis.text.x = element_blank(), legend.position = 'bottom') + guides(fill = guide_legend(ncol = 2,nrow = 4, title.position = 'top'))
```
