...
p2 <- Workshops %>% filter(workshop != "I will not be attending the workshops") %>% group_by(day) %>% tally()%>% ggplot(., aes(x = day, y = n, fill = factor(day))) + geom_bar(stat = 'identity') + theme_bw() + geom_hline(yintercept = 140, colour = "red") + theme( axis.ticks.x = element_blank(), legend.position = 'none') + geom_text(aes(label = n, y=n),stat="identity", position=position_dodge(0.9),vjust=-0.2)
...
### Total Registrants Remaining - Workshops

```{r}



p2 <- Workshops %>% mutate(date = lubridate::mdy_hms(Time), attend = 1)  %>% group_by(day, workshop) %>% mutate(remain = 60 - cumsum(attend)+1)%>% ggplot(., aes(x = date,y = remain, by = workshop, colour = workshop)) + geom_line() +  theme_bw() + geom_hline(yintercept = 60, colour = "red") + scale_y_continuous(limits = c(0, 60)) + facet_grid(day~.) + geom_hline(yintercept = 20, lty = 'dashed')

p2
```

### Attend Talks Spots Remaining

```{r}
resbaz %>% mutate(attend = if_else(Talks == "I will attend the talks/demonstrations on July 6 .", 1, 0)) %>% mutate(date = lubridate::mdy_hms(Time)) %>% mutate(remain = 400 - cumsum(attend)+1)%>% ggplot(., aes(x = date,y = remain)) + geom_line() +  theme_bw() + geom_hline(yintercept = 400, colour = "red") + scale_y_continuous(limits = c(0, 400))
```
...
```{r}
resbaz %>% ggplot(., aes(x = Discipline, fill = Discipline)) + geom_bar() + theme_bw()+ theme(axis.text.x = element_blank())
```
